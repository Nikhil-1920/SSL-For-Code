...
def batch(overwrite: bool = False, num_repos: int = 10):
    ...
    # Download zips (limited to num_repos)
    for i, r in monit.enum(f"Download {min(len(repos), num_repos)} repos", repos[:num_repos]):
...
def main():
    ...
    parser.add_argument('--num-repos', type=int, default=10, help='Number of repositories to download (default: 10)')
    args = parser.parse_args()

    try:
        batch(num_repos=args.num_repos)
